...
import (
	...
	service "github.com/VlasovArtem/hob/src/payment/service"
...
func (_m *PaymentService) FindByGroupId(id uuid.UUID, limit int, offset int, from *time.Time, to *time.Time) []model.PaymentDto {
...
func (_m *PaymentService) Transactional(db *gorm.DB) service.PaymentService {
	ret := _m.Called(db)

	var r0 service.PaymentService
	if rf, ok := ret.Get(0).(func(*gorm.DB) service.PaymentService); ok {
		r0 = rf(db)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(service.PaymentService)
		}
	}
